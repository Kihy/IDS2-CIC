...
def plot_particle_position(pos_history, gbest, animation_path):
    designer = Designer(
                limits=[(0, 10), (0, 1514)], label=["n", "p"]
            )
...
def packet_gen(ori_pkt, traffic_vector):
    ...
    # packet.show()
        ...
        packet[IP].add_payload(UDP(sport=int(traffic_vector[4]),dport=int(traffic_vector[6])))
        ...
        packet[IP].add_payload(ARP(psrc=int(traffic_vector[1]),pdst=int(traffic_vector[2])))
...
def build_craft(timestamp, num_craft, prev_pkt_time, max_craft, max_pkt_size=None, min_pkt_len=None):
    ...
    for i in range(num_particles):
        ...
        t[i][:num_craft[i]]=np.linspace(prev_pkt_time, timestamp[i], num_craft[i]+1, endpoint=False)[1:]
        t[i][:num_craft[i]]=np.around(t[i][:num_craft[i]], decimals=6)


    # t=np.random.uniform(prev_pkt_time,np.expand_dims(timestamp, axis=1), size=(num_particles,max_craft))
    ...
    if not isinstance(min_pkt_len, int):
        l=np.tile(np.expand_dims(max_pkt_size,axis=1), (1,max_craft))
    else:
        l=np.random.randint(min_pkt_len, max_pkt_size,size=(num_particles,max_craft))
...
def f(x, decision_func, adv_feature, decision_type, FE, max_time_window, prev_pkt_time, max_pkt_size, traffic_data, max_craft_pkt, min_pkt_len, db, verbose=False):
    ...
    if n_dims==3:
        ...
        craft_batch=build_craft(timestamps, num_craft, prev_pkt_time, max_craft_pkt,max_pkt_size=pkt_sizes,min_pkt_len=pkt_sizes)

    if n_dims==2:
        craft_batch=build_craft(timestamps, num_craft, prev_pkt_time, max_craft_pkt, max_pkt_size=max_pkt_size, min_pkt_len=min_pkt_len)
    ...
    if decision_type=="adv":
        min_dists=tf.keras.losses.mean_squared_error(features, np.tile(adv_feature,(n_particles,1)))
    if decision_type=="kitsune":
...
def craft_adversary(mal_pcap, init_pcap, adv_pcap, mal_pcap_out, decision_type, threshold, model_path, optimizer=None,init_count=0, netstat_path=None, mutate_prob=-1,base_offset=0, log_file=None, n_dims=2, max_time_window=60, max_craft_pkt=5, num_benign_sample=100, max_pkt_size=655 , max_adv_pkt=None, adv_csv_file=None, animation_folder=None):
    ...
    meta_file=open("../experiment/traffic_shaping/crafted_meta_{}.csv".format(max_time_window),"w")
    ...
    # init with benign packets
        ...
        if create_init:
            init_pcap_file=PcapWriter("../experiment/traffic_shaping/init_pcap/wiretap.pcap")
        ...
        while pkt_index < init_count:
            ...
            if traffic_data == []:
                ...
                np.savetxt(output_csv, [np.full(features.shape,-1)], delimiter=",")
                continue
            ...
            meta_file.write(",".join([str(pkt_index),str(packet.time), "init\n"]))
            craft_pcap.write(packet)
    ...
    while adv_pkt_index<max_adv_pkt:
        ...
        if tmp_pkt.haslayer(UDP):
            tmp_pkt[UDP].remove_payload()
        if tmp_pkt.haslayer(ARP):
        ...
        features=feature_extractor.nstat.update_dummy_db(*(traffic_data),dummy_db,False)

        rmse=decision_func(features)
        ...
        if rmse<threshold:
            ...
            # feature_extractor.dummy_nstat.updateGetStats(*traffic_data)
            #
            meta_file.write(",".join([str(pkt_index),str(packet.time),"attacker_low\n"]))
            prev_pkt_time=float(packet.time)
            continue

        log_file.write(pformat(features))
        original_time=traffic_data[-2]
        print("original RMSE", rmse)
        log_file.write("original RMSE: {}\n".format(rmse))
        print("original time", original_time)
        log_file.write("original time: {}\n".format(original_time))
        malicious_cost+=rmse
        ...
        # PSO
        args={"FE":feature_extractor,"adv_feature":adv_feature, "min_pkt_len":min_pkt_len, "decision_type":decision_type, "max_time_window":max_time_window, "prev_pkt_time":prev_pkt_time, "decision_func":decision_func, "max_pkt_size":max_pkt_size, "traffic_data":traffic_data,"max_craft_pkt":max_craft_pkt,"db":db}
        ...
        print("optimizing mal file index: ", pkt_index-total_craft-init_count)
        print("optimizing craft file index: ",pkt_index)
        print("bounds: {}".format(pformat(bounds)))
        log_file.write("optimizing mal file index: {}\n".format(pkt_index-total_craft-init_count))
        log_file.write("optimizing craft file index: {}\n".format(pkt_index))
        log_file.write("adversarial packet number: {}\n".format(adv_pkt_index))
        log_file.write("bounds: {}\n".format(pformat(bounds)))

        iterations=30

        if optimizer=="pso":
            cost,pos,aux, std, pos_history=optimize(options, n_dims, iterations, f, args,bounds, original_time,mutate_prob=mutate_prob)
        if optimizer=="de":
            cost,pos,aux,std, pos_history=differential_evolution(n_dims, iterations, f, args, bounds, original_time)

        log_file.write("best cost: {}, best pos: {}\n".format(cost, pos))
        ...
        if cost>threshold:
            ...
            if rmse-cost<0.1:
                animaition_path=animation_folder+"_{}.gif".format(pkt_index-total_craft-init_count)
                plot_particle_position(pos_history, pos, animaition_path)
        ...
        for i in range(num_craft):
            ...
            traffic_vector=traffic_data[:-2]+[aux[i],int(aux[i+max_craft_pkt])]
            ...
            if write_to_csv:
                np.savetxt(output_csv, [features], delimiter=",")
            meta_file.write(",".join([str(pkt_index),str(craft_packet.time),"craft\n"]))
        ...
        craft_pcap.write(packet)
        malicious_pcap.write(packet)
        meta_file.write(",".join([str(pkt_index),str(packet.time),"malicious\n"]))
        adv_pkt_index+=1
        prev_pkt_time=pos[0]

    report={}
    if adv_pkt_index==0:
        report["av_time_delay"]=0
        report["av_num_craft"]=0
        report["adv_mal_ratio"]=0
        report["av_cost_after"]=0
        report["av_cost_before"]=0
        report["av_std"]=0
    else:
        report["av_time_delay"]=(offset_time-base_offset)/adv_pkt_index
        report["av_num_craft"]=total_craft/adv_pkt_index
        report["adv_mal_ratio"]=(pkt_index-init_count-total_craft)/adv_pkt_index
        report["av_cost_after"]=total_cost/adv_pkt_index
        report["av_cost_before"]=malicious_cost/adv_pkt_index
...
def get_decision_func(decision_type, model_path):
    ...
    if decision_type=="adv":
        model=tf.keras.models.load_model("../models/surrogate_ae_video_ho.h5")
...
def optimize(options, n_dims, iterations, f, args,bounds, original_time, clamp=None, mutate_prob=-1):
    ...
    mutation_factor=0.8
    crossp=0.7
    mutation_candidates = [[idx for idx in range(n_particles) if idx != i] for i in range(n_particles)]
    mutation_candidates=np.array(mutation_candidates)

    swarm=create_swarm(n_particles=n_particles, dimensions=n_dims, options=options, bounds=bounds, discrete_index=1)
    print(swarm.position.dtype)
    ...
    for i in pbar:
        ...
        swarm.current_cost, swarm.current_aux = f(swarm.position, **args, verbose=False) # Compute current cost
        # file.write("{}".format(i)+"\n")
        ...
        pos_history.append(np.vstack((swarm.position[:,1:], swarm.best_pos[:,1:])))

        # binomially mutate
        if np.random.rand()<mutate_prob:
            tmp_pos=swarm.position
            swarm.trial_pos=topology.mutate_swarm(swarm, mutation_factor, crossp, mutation_candidates, bounds)
            ...
            swarm.position,swarm.current_cost, swarm.current_aux=topology.compute_mbest(swarm)
            # print("-"*50)


        swarm.pbest_pos, swarm.pbest_cost, swarm.pbest_aux, swarm.pbest_iter = topology.compute_pbest(swarm, i) # Update and store
        ...
        swarm.best_pos, swarm.best_cost, swarm.best_aux, swarm.best_index= topology.compute_gbest_local(swarm,2,4)
            # best_iter=i
        # file.write(pprint.pformat(swarm.best_pos)+"\n")

        # Part 3: Update position and velocity matrices
        # Note that position and velocity updates are dependent on your topology
        swarm.velocity = topology.compute_velocity(swarm,bounds=bounds, clamp=clamp, iter=i)
        if np.random.rand()<0.5:
            strat="random"
        else:
            strat="nearest"
        swarm.position = topology.compute_position(swarm, bounds=bounds, bh=BoundaryHandler(strategy=strat))
